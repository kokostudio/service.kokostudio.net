		...
		if ($scriptblock == Ucdn::SCRIPT_KHMER) {
			...
			for ($i = $base; $i < $end - 1; $i++) { /* KHMER_FIX_1 From $start (not base) */
				if (self::is_halant_or_coeng($info[$i]) && self::is_ra($info[$i + 1]['uni'])) {
					...
					/* Mark the subsequent stuff with 'cfas'.  Used in Khmer.
					 * Read the feature spec.
					 * This allows distinguishing the following cases with MS Khmer fonts:
					 * U+1784,U+17D2,U+179A,U+17D2,U+1782  [C+Coeng+RO+Coeng+C] => Should activate Cfas
					 * U+1784,U+17D2,U+1782,U+17D2,U+179A  [C+Coeng+C+Coeng+RO] => Should NOT activate Cfas
					 */
					for ($j = ($i + 2); $j < $end; $j++) {
						$info[$j]['mask'] |= self::FLAG(self::Cfas);
					}
